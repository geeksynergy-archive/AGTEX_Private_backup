function twentytypeCard(img_temp,col,row,card,card_select,card_save,handles)
...
res_Image=double(vec2mat(img_temp,20));
    res_Image(res_Image==255)=1;
    res_Image(res_Image==0)=255;
c=0;
card=cell(length(img_temp(:,1)),1);
step=1:row:length(res_Image);
