...
% Card Read Start from top, bottom, left, right
popValue = get(handles.popupmenu18,'Value');

switch popValue
    case 1 
        img_temp; % Top-Left
    case 2
        img_temp = fliplr(img_temp); % Top-Right
    case 3
        img_temp = flipud(img_temp);% Bottom-Left
    case 4 
        img_temp = fliplr(img_temp); % Bottom-Right
        img_temp = flipud(img_temp);
end
img_temp=squeeze(img_temp(:,:,1));
...
if card_select.peg3==0 % Number of peg=2
    ...
    %         card_disp(card_select.no_row,card_select.peg3)=5;
    %     else
    %         template(round(card_select.no_row/2),card_select.peg1)=5;
    %         template(round(card_select.no_row/2),card_select.peg2)=5;
    ...
    if rem(card_select.no_row,2)==0 %Peg for cards with even number of rows
        ...
        for i=1:length(img_card(:,1))
            ...
            card_disp_emg([1:2:end/2-3],card_select.peg1)=img_temg1(1:end/2);
            card_disp_emg([end/2+3:2:end],card_select.peg1)=img_temg1(end/2+1:end);
            card_disp_emg([1:2:end/2-3],card_select.peg3)=img_temg2(1:end/2);
            card_disp_emg([end/2+3:2:end],card_select.peg3)=img_temg2(end/2+1:end);
            %         end
            card_disp_emg(card_select.no_row,card_select.peg1)=5;
            card_disp_emg([card_select.no_row-1 card_select.no_row+1],card_select.peg1)=0;
            card_disp_emg(card_select.no_row,card_select.peg2)=5;
            card_disp_emg([card_select.no_row-1 card_select.no_row+1],card_select.peg2)=0;
            card_disp_emg(card_select.no_row,card_select.peg3)=5;
            card_disp_emg([card_select.no_row-1 card_select.no_row+1],card_select.peg3)=0;
            card_disp{i}=card_disp_emg;
        end
    else
        %         if card_select.emg==1 % Emergency enabled
        ...
        for i=1:length(img_card(:,1))
            ...
            template([1:floor(end/2)-1],card_select.peg1)=img_temg1(1:end/2);
            template([round(end/2)+2:end],card_select.peg1)=img_temg1(end/2+1:end);
            template([1:floor(end/2)-1],card_select.peg3)=img_temg2(1:end/2);
            template([round(end/2)+2:end],card_select.peg3)=img_temg2(end/2+1:end);
            %         end
            template(round(card_select.no_row/2),card_select.peg1)=5;
            template(round(card_select.no_row/2),card_select.peg2)=5;
            template(round(card_select.no_row/2),card_select.peg3)=5;
            template([round(card_select.no_row/2) round(card_select.no_row/2)+1 round(card_select.no_row/2)-1],card_select.peg1)=0;
            template([round(card_select.no_row/2) round(card_select.no_row/2)+1 round(card_select.no_row/2)-1],card_select.peg2)=0;
            template([round(card_select.no_row/2) round(card_select.no_row/2)+1 round(card_select.no_row/2)-1],card_select.peg3)=0;
...
for i=1:length(card)
    ...
    if rem(card_select.no_row,2)==0
        ...
        if(card_select.peg3==0)
            ...
            card_disp1([1:2:end],[1:card_select.peg1-1 card_select.peg1+1:card_select.peg2-1 card_select.peg2+1:card_select.peg3-1 card_select.peg3+1:end ])=temp_template([1:end],[1:card_select.peg1-1 card_select.peg1+1:card_select.peg2-1 card_select.peg2+1:card_select.peg3-1 card_select.peg3+1:end]);
        end
